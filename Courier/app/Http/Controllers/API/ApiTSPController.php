...
class ApiTSPController extends Controller {
    ...
    public function store(Request $request)
    {
        ...
        if($user !== null && !($user->isNotConfirmed())) {
            ...
            $parcelStrInput = $request->input('addresses');
            $parcelInput = explode('@-@', $parcelStrInput);
            $firstInput = explode('|', Input::get('first'));
            $option = $request->input('option');
            $edges = $request->input('edges');
            if($parcelStrInput !== null && strlen($parcelStrInput) > 0 && strlen($firstInput[0]) > 0) {
                ...
                if($numberOfDiffAddresses >= 3 && (($numberOfDiffAddresses <= 12 && $edges == "real") || ($numberOfDiffAddresses <= 60 && $edges == "linear"))){
                    try {
                        ...
                        return response()->json([
                            ...
                            'totalTime' => $totalTime,
                            'pointAddresses' => array_column($TSPdata, 'address'),
                            'pointXs' => $this->setNumbersToTextInArray(array_column($TSPdata, 'x'), 1),
                            'pointXGraphs' => array_column($TSPdata, 'xGraph'),
                            'pointYs' => $this->setNumbersToTextInArray(array_column($TSPdata, 'y'), 1),
                            'pointYGraphs' => array_column($TSPdata, 'yGraph'),
                            'pointDistances' => array_column($TSPdata, 'distance'),
                            'pointDurations' => $this->setNumbersToTextInArray(array_column($TSPdata, 'duration'), 2),
                            'options' => $this->checkSettings($option, $edges),
                            'result' => 'true']);
                    } catch (Exception $e){
                        return response()->json([
                            'message' => "Błąd aplikacji: ".$e->getMessage()." [".$e->getCode()."]",
                            'result' => 'false']);
                    }
                } elseif ($edges == "real") {
                    return response()->json([
                        'message' => "Możesz wybrać przesyłki, dla których łączna ilość różnych adresów wynosi od 3 do 12! (dla danych ustawień)
                            Wybrano: ".$numberOfDiffAddresses." różnych adresów.",
                        'result' => 'false']);
                } else {
                    return response()->json([
                        'message' => "Możesz wybrać przesyłki, dla których łączna ilość różnych adresów wynosi od 3 do 60! (dla danych ustawień)
                            Wybrano: ".$numberOfDiffAddresses." różnych adresów.",
                        'result' => 'false']);
                }
            } elseif ($option == null || strlen($option) <= 0 || $edges == null || strlen($edges) <= 0) {
                return response()->json([
                    'message' => "Musisz wybrać opcje wyszukiwania!",
                    'result' => 'false']);
            } elseif (strlen($parcelInput[0]) <= 0){
                return response()->json([
                    'message' => "Musisz wybrać przesyłki!",
                    'result' => 'false']);
            } else/*if (strlen($firstInput[0]) <= 0) */ {
                return response()->json([
                    'message' => "Musisz wybrać pierwszą przesyłkę!",
                    'result' => 'false']);
            }
        } else {
            return response()->json([
